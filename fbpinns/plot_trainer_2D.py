...
def _plot_test_im(u_test, xlim, ulim, n_test, it=None):
    ...
    im = ax.imshow(u_test.T,  # transpose as jnp.meshgrid uses indexing="ij"
                   ...
                   cmap="bwr",
                   vmin=((ulim[0] / 5) + 0.1),
                   vmax=(-((ulim[0] / 5) + 0.1)), interpolation='bicubic')
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.1)
    cbar = plt.colorbar(im, cax=cax)

    # Set the colorbar ticks to have a consistent number of ticks
    cbar.locator = ticker.MaxNLocator(nbins=6)  # Change nbins to the desired number of ticks
    cbar.update_ticks()

    ax.set_xlim(xlim[0][0], xlim[1][0])
    ax.set_ylim(xlim[0][1], xlim[1][1])
    ax.set_aspect("equal")
    # Set the number of ticks for both x and y axes to 2
    ax.xaxis.set_major_locator(ticker.MaxNLocator(nbins=2))  # Number of ticks on the x-axis
    ax.yaxis.set_major_locator(ticker.MaxNLocator(nbins=2))  # Number of ticks on the y-axis
