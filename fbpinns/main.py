...
def _x_random(subdomain_xs, batch_size, device):
    ...
    s = torch.tensor([[x.min(), x.max()] for x in subdomain_xs], dtype=torch.float32, device=device).T.unsqueeze(
        1)  # (2, 1, nd)
    x_random = s[0] + (s[1] - s[0]) * torch.rand((np.prod(batch_size), len(subdomain_xs)),
                                                 device=device)  # random samples in domain
    return x_random
...
def _x_mesh(subdomain_xs, batch_size, device):
    ...
    x_mesh = torch.stack(torch.meshgrid(*x_mesh), -1).view(-1,
                                                           len(subdomain_xs))  # nb torch.meshgrid uses np indexing="ij"
    return x_mesh
...
class FBPINNTrainer(_Trainer):
    ...
    def _train_step(self, models, optimizers, c, D,
                    i):  # use separate function to ensure computational graph/memory is released
        ...
        for im, i1 in D.active_ims:
            ...
            for iseg in D.m[im]:
                ...
                for im2, j1, j2, j3 in D.s[iseg]:
                    ...
                    if im2 == im:
                        yj = [t[j2:j3] for t in yj]  # get appropriate segment
                    else:
    ...
    def _test_step(self, x_test, yj_true, xs, yjs, yjs_sum, models, c, D, i, mstep, fstep, writer,
                   yj_test_losses):  # use separate function to ensure computational graph/memory is released
        ...
        if (i + 1) % c.TEST_FREQ == 0:
            ...
            # save figures
            fs = plot_main.plot_FBPINN(x_test, yj_true, xs, yjs, yjs_sum, yj_full, yjs_full, ys_full_raw,
                                       yj_test_losses, c, D, i + 1)
    ...
    def train(self):
        ...
        for i, active in enumerate(A):
            ...
            for im, i1 in D.active_fixed_neighbours_ims: fstep += models[im].flops(
                xs[i1].shape[0])  # record number of FLOPS
            ...
            if (i + 1) % c.SUMMARY_FREQ == 0:
                ...
                # test step
                yj_test_losses = self._test_step(x_test, yj_true, xs, yjs, yjs_sum, models, c, D, i, mstep, fstep,
                                                 writer, yj_test_losses)
...
class PINNTrainer(_Trainer):
    ...
    def _train_step(self, model, optimizer, c, i, mu, sd,
                    device):  # use separate function to ensure computational graph/memory is released
    ...
    def _test_step(self, x_test, yj_true, x, yj, model, c, i, mstep, fstep, writer,
                   yj_test_losses):  # use separate function to ensure computational graph/memory is released
    ...
    def train(self):
        ...
        xmin, xmax = torch.tensor([[x.min(), x.max()] for x in c.SUBDOMAIN_XS], dtype=torch.float32,
                                  device=device).T
        mu = (xmin + xmax) / 2;
        sd = (xmax - xmin) / 2
        ...
        for i in range(c.N_STEPS):
            ...
            if (i + 1) % c.SUMMARY_FREQ == 0:
                ...
                # test step
                yj_test_losses = self._test_step(x_test, yj_true, x, yj, model, c, i, mstep, fstep, writer,
                                                 yj_test_losses)
                # yj_test_losses = self._test_step_no_true_solution(x_test, x, yj, model, c, i, mstep, fstep, writer,
                #                                                   yj_test_losses)
