...
class RectangularDomainND(Domain):
    ...
    @staticmethod
    def boundary_near_rectangle(key, sampler, xmin, xmax, batch_shape):
    ...
    @staticmethod
    def boundary_near_triangle(key, sampler, xmin, xmax, batch_shape):
        def generate_triangle_boundary_points(center, side_length, num_points_per_side):
            ...
            for i in range(3):
                ...
                side_points = np.linspace(start_point, end_point, num_points_per_side + 1)
                boundary_points.extend(side_points[:-1])  # Exclude the last point to avoid duplicates

            return np.array(boundary_points)

        # Generate boundary points on the triangle
        num_boundary_points = batch_shape[0] * batch_shape[1]
        boundary_points = generate_triangle_boundary_points([0, -0.5], 0.5, num_boundary_points)

        # Sample points near the boundary points
        sampling_distance = 0.01
        num_samples_per_point = max(1, batch_shape[1] // num_boundary_points)
        sampled_points = RectangularDomainND.sample_near_boundary(
            np.array(boundary_points), sampling_distance, num_samples_per_point, xmin, xmax
        )

        # Filter and return the sampled points
        x_filtered = RectangularDomainND.foo(
            np.array(sampled_points), xmin[2], xmax[2], batch_shape[2]
        )
